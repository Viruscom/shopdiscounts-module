...
@section('content')
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card">
                    <div class="card-header">EDIT {{$discount->type_id}}
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="{{route('discounts.update',['id'=>$discount->id])}}">
                                ...
                                <div class="col-md-6">
                                    <select id="applies_to" class="form-control @error('applies_to') is-invalid @enderror" name="applies_to" required autofocus onchange="showInput(this)">
                                        @foreach($applications as $application)
                                            <option value="{{$application}}" {{$discount->applies_to==$application ? 'selected':''}}>{{__('applies to ').$application}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <select id="brand_id" class="should-be-required  form-control @error('brand_id') is-invalid @enderror" name="brand_id" autofocus>
                                        @foreach($brands as $brand)
                                            <option value="{{$brand->id}}" {{$discount->brand_id==$brand->id ? 'selected':''}}>{{__('Brand ').$brand->id}}</option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-md-6">
                                    <select multiple id="categories_ids" class="should-be-required form-control @error('categories_ids.*') is-invalid @enderror" name="categories_ids[]" autofocus>
                                        @foreach($categories as $category)
                                            <option value="{{$category->id}}" {{$discount->categories->contains($category->id) ? 'selected':''}}>{{__('Category ').$category->id}}</option>
                                        @endforeach
                                    </select>
                            ...
                            <div style="display:none" class="form-group row applies-to-input applies-to-input-{{Modules\ShopDiscounts\Entities\Discount::$ABOVE_ORDER_VALUE_APPLICATION}}">
                                ...
                                <div class="col-md-6">
                                    <input id="order_value" type="number" min="0" step="0.01" class="should-be-required form-control @error('order_value') is-invalid @enderror" name="order_value" value="{{ $discount->order_value }}" autocomplete="order_value" autofocus>
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-6">
                                    <input id="promo_code" type="text" class="form-control @error('promo_code') is-invalid @enderror" name="promo_code" value="{{ $discount->promo_code }}" autocomplete="promo_code" autofocus>
